...
> [!principle] 핵심 원칙
> 인간이 위키를 포기하는 주된 이유는 유지보수 부담(교차 참조, 요약 최신화, 일관성 유지)이다. LLM이 이 비용을 대신 처리함으로써 지식이 흩어지지 않고 구조화된 형태로 축적될 수 있다.
...
|---|---|---|
| 처리 시점 | 질의 시 | 수집 시 |
| 지식 형태 | 원본 청크 | 구조화된 마크다운 |
| 교차 참조 | 없음 | LLM이 능동적으로 추가 |
| 유지보수 | 자동 | LLM이 담당 |
