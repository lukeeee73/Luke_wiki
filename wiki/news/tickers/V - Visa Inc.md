---
title: "V - Visa Inc. — Routine News Log"
created: 2026-05-16
updated: 2026-07-18
domain: finance
type: claim
weight: reference
confidence: low
tags: [routine-news, watchlist, V]
sources: []
---

# V - Visa Inc. — Routine News Log

**Visa Inc.** · Financial Services · 금융 · 경쟁사: MA, AXP, PYPL

## 회사 소개

신용카드·직불카드 결제 네트워크 세계 1위. 카드를 직접 발급하지 않고, 전 세계 어디서든 결제가 이루어질 때마다 소액의 수수료를 가져가는 구조다. 거의 모든 신용카드사·은행이 비자 네트워크를 이용하기 때문에 안정적이고 수익성이 높다.

> [!info] 자동 수집 노트
> 이 페이지는 `indicator_dashboard` 의 `daily-market-analysis` 루틴이 매일 누적한다. 신규 항목은 모두 `[!claim]` (confidence: low) 로 들어오고, 후속 일자에 [사실 누적] 또는 [반증] 으로 분류된다. 폴더 규칙: [news/README.md](../README.md).

---

## 미해결 가설 (Open Claims)

루틴이 매일 이 목록을 읽고, 오늘 뉴스가 가설을 (verified / refuted / pending / aged-out) 중 어디에 해당하는지 판정한다.

<!-- OPEN_CLAIMS_START -->
- [ ] **2026-06-27**: $380억 스와이프 수수료 합의 예비 승인 — 수수료 인하의 실제 실적 영향 정도 (pending) ※ 2026-07-18: 아직 최종 승인 전, 대형 유통업체들의 반대 의견 제출이 이어지는 중 — 계속 모니터링
- [ ] **2026-05-17**: UK FCA V·MA·PayPal 디지털 지갑 경쟁 조사 — 규제 결과 대기 (pending) ※ 2026-06-06: 새로운 규제 결과 뉴스 없음, 계속 모니터링
- [x] **2026-05-17**: Visa Agentic Ready 글로벌 확장 — AI 결제 선점 전략 (verified) ※ 2026-06-06: Canton Network 스테이블코인 결제 파일럿(Blockonomi) — AI/블록체인 결제 인프라 선점 전략 확장 확인
[ ] **2026-06-13**: V·OpenAI 결제 통합 — 실제 AI 에이전트 거래량 및 수수료 기여도 확인 필요 (pending)
- [ ] **2026-07-18**: 비자의 스테이블코인 플랫폼(VSP)이 실제 은행·핀테크 고객 확보로 이어질까 — 이렇게 확인된다: 신규 파트너사 발표 또는 7/28 3분기 실적에서 스테이블코인 관련 거래량·매출 언급 여부 (시한 7/28)
<!-- OPEN_CLAIMS_END -->

---

## 사실 누적 (Verified Facts)

독립 Tier-1 매체 2 곳 이상이 보고하거나, 회사 IR / 공시로 확정된 사실만 모음.

<!-- FACTS_START -->
> [!fact] (확인: Reuters 2026-06-09 + Fortune 2026-06-08) Visa·Mastercard $380억 스와이프 수수료 합의 법원 예비 승인
> 복수 Tier-1 매체가 동일 사실 보도. 수수료 5년간 0.1%p 인하 포함.

> [!fact] (확인: Motley Fool 2026-06-04 + Blockonomi 2026-06-04) Visa FQ2 2026 매출·EPS 컨센서스 상회, Canton Network 스테이블코인 결제 파일럿 착수
> 복수 Tier-1 매체 보도. FQ2 실적 상회 및 블록체인 결제 인프라 확장 동시 확인.
> [!fact] (확인: Yahoo Finance 2026-06-10 + TIKR 2026-06-11) V FQ2 2026 순매출 $112억 YoY +17%
> 두 독립 매체 확인. 비자의 FQ2 순매출이 13년 만에 최고 성장률 +17%를 기록. CEO '2022년 이후 최강 성장'이라고 공식 확인.

> [!fact] (확인: Visa IR 2026-07-16 + Bloomberg 2026-07-16) 비자 스테이블코인(달러 등에 가치를 고정한 암호화폐) 플랫폼 VSP 출시
> 회사 공시와 Tier-1 매체가 동시 확인. 은행·핀테크·크립토 기업이 스테이블코인을 발행·관리할 수 있는 플랫폼으로, 우선 Open USD를 지원한다. 왜 중요: 비자가 스테이블코인 인프라 제공자 역할까지 선점하며 결제망 해자를 넓히려는 시도다.
<!-- FACTS_END -->

---

## 일자별 기록 (역순)

<!-- DAILY_START -->

### 2026-07-18 (토) — 신호등: 🟢 순풍 (+0.17)

**세 줄 요약**
1. 비자가 은행·핀테크용 스테이블코인(달러 등에 가치를 고정한 암호화폐) 발행·관리 플랫폼 'VSP'를 출시했고, 발표일 주가는 +1.7% 올랐다.
2. 클리어스트리트가 비자에 대해 신규 커버리지(증권사가 새로 그 종목 분석을 시작하며 투자의견을 내는 것)를 매수의견으로 개시했고, 바클레이즈도 3분기 실적(7/28) 전에 목표주가를 올렸다.
3. 다만 DOJ의 직불카드 반독점 소송과 $380억 스와이프 수수료(가맹점이 카드 결제를 받을 때 카드사에 내는 수수료) 합의는 아직 마무리되지 않아 규제 불확실성은 남아있다.

**뉴스 브리핑**

**① 비자, 스테이블코인 발행·관리 플랫폼 'VSP' 출시** 🟢
- **무슨 일**: 비자가 은행·핀테크·크립토 기업이 스테이블코인을 발행하고 지갑으로 관리할 수 있는 플랫폼(VSP)을 공개했다. 우선 Open USD라는 신규 스테이블코인을 지원한다.
- **왜 중요**: 카드 결제 수수료가 핵심 수익원인 비자가, 스테이블코인이라는 잠재적 대체 결제 수단의 "인프라 제공자" 자리까지 선점하려는 시도다.
- **주가에 의미**: 단기 호재. 발표일 주가 +1.7%. 경쟁 스테이블코인 발행사(Circle) 주가는 반대로 떨어졌다.
- 출처: [Bloomberg](https://www.bloomberg.com/news/articles/2026-07-16/visa-is-expanding-its-crypto-push-with-new-stablecoin-platform) · 2026-07-16

**② 클리어스트리트, 비자 매수의견 개시(목표가 $403)** 🟢
- **무슨 일**: 증권사 클리어스트리트가 비자를 신규 커버리지 매수의견으로 개시하며 목표주가 $403을 제시했다. 같은 날 마스터카드도 매수의견으로 함께 커버리지를 시작했다.
- **왜 중요**: 결제 네트워크 업종 전체에 대한 증권가의 신뢰가 높다는 신호이며, 영업이익률 67% 이상·잉여현금흐름 전환율 등 비자의 수익성이 다시 부각됐다.
- **주가에 의미**: 완만한 호재. 다만 마스터카드도 동시에 좋은 평가를 받아 비자만의 차별화 신호는 아니다.
- 출처: [Investing.com](https://www.investing.com/news/analyst-ratings/clear-street-initiates-visa-stock-coverage-with-buy-rating-on-payments-growth-93CH-4795349) · 2026-07-16

**③ 바클레이즈, 3분기 실적 앞두고 목표주가 $420로 상향** 🟢
- **무슨 일**: 바클레이즈가 비자 목표주가를 $420로 제시하며 신규 커버리지를 시작했다. 7/28 3분기 실적 발표를 앞두고 컨센서스(애널리스트 전망치 평균) 상회를 기대하는 분위기다.
- **왜 중요**: 비자는 최근 4개 분기 연속 시장 예상치를 넘는 실적을 냈고, 이번에도 비슷한 흐름이 이어질 것으로 보는 시각이 많다는 뜻이다.
- **주가에 의미**: 실적 발표(7/28) 전 기대치를 높이는 재료 — 실제 실적이 이 기대에 못 미치면 오히려 실망 매물이 나올 수 있다.
- 출처: [Barchart](https://www.barchart.com/story/news/3252767/what-you-need-to-know-ahead-of-visas-earnings-release) · 2026-07-10

**경쟁 구도 한눈에**
- 마스터카드도 같은 날(7/16~17) 클리어스트리트로부터 매수의견(목표가 $617)을 받아, 카드 네트워크 듀오폴리(2강 체제) 전반에 대한 우호적 시각이 확인된다.
- 페이팔은 스트라이프·어드벤트로부터 $530억 규모 인수 제안을 받았다는 보도(7/15)로 주가가 +19% 급등했다. 다만 애널리스트들은 페이팔·스트라이프의 결제도 상당수 비자·마스터카드 카드망 위에서 처리되는 만큼, 이번 인수설이 비자 거래량에 직접 타격을 주지는 않을 것으로 본다.

**앞으로 지켜볼 것**
- [ ] 7/28 (비자 3분기 실적 발표) — 확인할 것: 컨센서스 EPS $3.22 상회 여부, 스테이블코인 플랫폼(VSP) 관련 언급 여부
- [ ] 7/28 (VSP 신규 파트너 확대 여부) — 확인할 것: Open USD 외 스테이블코인·은행 고객 추가 참여 발표 여부

### 2026-06-27

**narrative_score**: +0.17 (전회 +0.34, Δ -0.17)
**key_events**: $380억 스와이프 수수료 합의 6/9 예비 승인(수수료 5년간 0.1%p 인하)、Visa Payments Forum에서 AI·스테이블코인 신기술 발표(연환산 70억달러 처리)、Brale와 Canton 네트워크 기관 스테이블코인 정산 협력、Stripe·Coinbase와 신규 스테이블코인 플랫폼 추진 보도
**risks**: 스와이프 수수료 인하·가맹점 서차지 허용에 따른 수익성 압박、전미소매연맹(NRF) 등 합의 반대로 규제 불확실성 지속、스테이블코인이 장기적으로 카드 거래량 일부 대체할 위험

> [!claim] (출처: Reuters, 2026-06-09) US judge OKs Visa, Mastercard $38 billion swipe fee settlement
> 판사가 $380억 스와이프 수수료 합의 예비 승인, 수수료 5년간 0.1%p 인하. impact: neutral / category: regulation

> [!claim] (출처: Visa IR, 2026-06-10) Visa Announces New AI, Stablecoin and Token Innovations at Visa Payments Forum
> AI·스테이블코인·토큰 신기술 발표, 스테이블코인 연환산 70억달러 처리. impact: + / category: product

> [!claim] (출처: Visa IR, 2026-06-04) Visa and Brale Explore Private Stablecoin Settlement for Institutional Payments
> Brale와 Canton 네트워크 기반 USD 스테이블코인 기관 정산 협력. impact: + / category: product

> [!claim] (출처: Fortune, 2026-06-08) Visa and Mastercard are planning to shake up the stablecoin market
> Stripe·Coinbase와 함께 신규 스테이블코인 플랫폼 추진 보도. impact: + / category: product

**경쟁사 동향**:
- MA: Mastercard도 동일한 $38B 스와이프 수수료 합의 당사자이자 스테이블코인 플랫폼 공동 추진 — 규제·소송 리스크와 스테이블코인 기회를 V·MA가 공유해 양사 간 차별화 약함
- PYPL: 스테이블코인·핀테크 결제 경쟁 가열 — Visa의 스테이블코인 정산 확대는 PYPL 등 핀테크 대비 결제 네트워크 해자 방어에 긍정적

### 2026-06-13

**narrative_score**: +0.34 (전회 +0.17, Δ +0.17)
**key_events**: FQ2 순매출 $112억 YoY +17% (13년 최고)、OpenAI 결제 통합、스테이블코인 파일럿
**risks**: 스테이블코인 확산 시 네트워크 수수료 모델 잠식、주가 52주 고점 대비 할인

> [!claim] (출처: TIKR, 2026-06-11) Visa Reports Q2 FY2026 Net Revenue +17% — Strongest in 13 Years
> V FQ2 순매출 $112억, YoY +17% — 2013년 이후 최고 성장률. impact: + / category: earnings

> [!claim] (출처: Yahoo Finance, 2026-06-10) Visa to Integrate Payments into OpenAI Platform for AI Agent Transactions
> V, OpenAI 플랫폼 결제 통합 — AI 에이전트 거래 지원. impact: + / category: product

> [!claim] (출처: Yahoo Finance, 2026-06-03) Visa Partners with Brale on Stablecoin Settlement on Canton Network
> V, Brale와 Canton Network 스테이블코인 정산 파일럿. impact: + / category: product

**경쟁사 동향**:
- MA: Agent Pay for Machines 출시 + 교환수수료 합의 예비 승인
- AXP: 신용카드 연체율 13% 15년 최고 — V 네트워크 모델 상대 우위 부각



### 2026-06-06

**narrative_score**: +0.17 (전회 +0.15, Δ +0.02)
**key_events**: Canton Network 스테이블코인 결제 파일럿、FQ2 매출·EPS 컨센서스 상회、주가 $323.82 +3.66%
**risks**: 쿠바 거래 중단(미미), 글로벌 소비 둔화

> [!claim] (출처: Blockonomi, 2026-06-04) Visa pilots stablecoin settlement via Canton Network with Brale
> 블록체인 결제 인프라 선점. impact: + / category: product

> [!claim] (출처: Motley Fool, 2026-06-04) Visa FQ2 2026 beats on revenue and EPS
> FQ2 컨센서스 상회. impact: + / category: earnings

> [!claim] (출처: Reuters, 2026-06-04) Visa and Mastercard suspend Cuba transactions June 6
> 쿠바 거래 중단(미미). impact: - / category: regulation

**경쟁사 동향**:
- MA: CFO 교체+스테이블코인 플랫폼, AXP: BofA 하향 + 연체율 15년 최고

### 2026-05-17

**narrative_score**: +0.15 (전회 —, Δ —)
**key_events**: FY2Q EPS +20%·매출 +17%、Agentic Ready 캐나다 확장、UK FCA 디지털 지갑 조사
**risks**: UK FCA 디지털 지갑 경쟁 조사 결과、PayPal 등 비은행 결제 경쟁

> [!claim] (출처: Motley Fool, 2026-05-09) Visa Q2 FY2026 EPS +20%, revenue +17%
> Visa FY2Q26 EPS +20%·매출 +17%, 소비자 심리 견조. impact: + / category: earnings

> [!claim] (출처: Yahoo Finance, 2026-05-05) Visa expands Agentic Ready to Canada
> AI 에이전트 결제 대응 프로그램 캐나다 확장. impact: + / category: product

> [!claim] (출처: Reuters, 2026-05-12) UK FCA competition probe into Visa, Mastercard, PayPal
> 영국 FCA 디지털 지갑 경쟁 조사 착수. impact: - / category: regulation

> [!claim] (출처: MarketBeat, 2026-05-12) Truist raises Visa target to $371
> Truist 목표주가 $361→$371 상향. impact: + / category: other

> [!claim] (출처: Yahoo Finance, 2026-05-15) Baupost initiates Visa stake
> 세스 클라만 바우포스트, Visa 신규 매수. impact: + / category: other

**경쟁사 동향**:
- MA: UK FCA 동일 디지털 지갑 조사 — 규제 리스크 공유, V 단독 불이익 없음 (impact for V: neutral)
- AXP: Q1 EPS +18%, 프리미엄 소비자 지출 강세 — Visa 결제 볼륨에 긍정 (impact for V: +)

<!-- DAILY_END -->